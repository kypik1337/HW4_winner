...
def save_to_csv(my_list):
    with open("60_meaters_women.csv", "w") as csvfile:
        writer = csv.DictWriter(
            csvfile, fieldnames=list(my_list[0].keys()), quoting=csv.QUOTE_NONNUMERIC)
        writer.writeheader()
        for row in my_list:
            writer.writerow(row)
    with open("60_meaters_women.csv") as f:
        print(f.read())
